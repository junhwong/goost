...
var (
	labelValueSepPatt = regexp.MustCompile(`[[:space:]\,\|;]`)
	labelValuePatt    = regexp.MustCompile(`\%?[[:word:]\-\.]+`) // %百分比
)

// 解析标签值, 符合独立无特殊字符。方便作为索引使用。
...
func ParseLabelValue(s string, ignoreInvalidSegment ...bool) ([]string, error) {
	...
	if len(ignoreInvalidSegment) > 0 {
		ignoreErr = ignoreInvalidSegment[len(ignoreInvalidSegment)-1]
	}
